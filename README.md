...
内置模块当前为 `quick-tools 2.3.2`，悬浮工具栏在全屏宿主中显式恢复指针交互，避免宿主的穿透层规则让按钮失去点击和悬停响应。此前已修复其实际目录名与扩展清单、配置中心和弹窗页面路径不一致的问题。现在所有加载路径、构建脚本、发布校验和文档统一指向 `modules/Kivowiki-Mods-quick-tools/`；本次 Core `1.6.0` 在此基础上增加标签式管理界面、市场自动发现、推荐、自定义源设置和文件夹导入能力。
