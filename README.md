...
├─ modules/quick-tools/      # 内置快捷工具模块项目
│  ├─ module.json
│  ├─ README.md
│  └─ src/                   # 入口、配置和后续功能分层
...
- 中间箭头：展开或收起更多工具，展开后会变成 X。
- 展开的夜间模式按钮：切换保留背景图的分层深色主题。

快捷工具配置支持分别决定按钮是否折叠、位置、按钮大小、边缘间距和实时预览。角色图鉴按钮默认固定显示在返回顶部按钮上方；模块本身不再绘制图鉴或重复请求角色 API，筛选、分页、资料跳转和后续站点升级全部由 KivoWiki 原生窗口负责。夜间模式设置按浏览器本地保存，刷新后保持；主题适配 Naive UI、Tailwind 内容色类、Markdown、首页消息气泡、角色地形图标与联系页文本。
