...
配置中心支持本地单文件 JSON、完整 ZIP 项目包，以及公开 GitHub/GitLab 仓库链接。远程仓库会下载默认分支 ZIP，然后进入与本地文件完全相同的预检。包和解压后总大小上限为 100 MB，单文件 32 MB，入口与配置脚本各 4 MB，文件数量最多 2048。资源存储在 IndexedDB，不占用 `chrome.storage.local` 的小配额。ZIP 根目录（或唯一项目目录）必须包含 `module.json`、`dependency.json` 或 `manifest.json`。
...
配置中心支持导出模块与依赖备份。备份是 JSON 文件，包含清单、入口、配置文件、设置和全部资源；可以导出单个包，也可以导出社区模块备份。备份改变了原始清单并加入本机设置，因此不会沿用作者原始签名。导入时会重新校验包类型、名称前缀、跨类型 ID 冲突、路径、入口、配置文件、重复文件和资源大小。
