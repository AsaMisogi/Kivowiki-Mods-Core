...
站点首页可能只请求一小批角色、文章或新闻，页面上的首批节点数量不代表 API 的总量。需要扩展首页列表时，应通过 `core-runtime.listAll` 按 `max_page` 分页读取，在客户端复用站点已有节点模板，并给新增节点加模块前缀标记，便于停用时完整清理。新闻轮播通常包含首尾过渡节点，不能简单把隐藏节点数量当成真实新闻数量；如果站点组件没有公开更新接口，应使用模块自己的轮播容器，不要修改 UI 框架的内部状态。
