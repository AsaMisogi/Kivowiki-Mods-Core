...
const buildPageScript = (entry, dependencySources = []) => {
  ...
  return [
    ...
    '  saveSettings(next) { if (!permissions.has("settings")) return Promise.reject(new Error("模块未获得设置保存权限")); settings = { ...(next || {}) }; context.settings = { ...settings }; return requestHost("save-settings", { settings }); },',
